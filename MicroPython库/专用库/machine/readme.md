...
- machine.`mem_backup`(region=0)
  ...
  总的大小（以字节为单位）是 `len(mem) * mem.itemsize`，其中 `len(mem)` 是元素的数量，`mem.itemsize` 是每个元素的大小。例如，在具有4个字大小寄存器的硬件上，`len(mem)` 为 4，`mem.itemsize` 为 4，总共16个字节。在具有4096字节字节可寻址备份SRAM的硬件上，`len(mem)`为4096，`mem.itemsize`为1。
...
counter = Counter(0, Pin(0, Pin.IN)) # 在引脚 0 创建计数器对象并开始计数
value = counter.value()              # 获取当前脉冲计数
```
...
- class machine.`Counter`(id, ...)
  ...
  返回给定id的 Counter 对象。id的范围取决于特定硬件及其软件移植，值0、1等通常用于选择硬件模块 #0、#1 等。
  
  其他参数被传递给下面描述的`init()`方法，并重新初始化Counter实例。

  在ESP32上，id对应于`PCNT`单元。
...
- Counter.`init`(src, *, ...)
  ...
  使用给定参数初始化并重置计数器：
  - `src` 指定输入引脚。对于具有预定义引脚的硬件，可以省略此参数。

  可能支持的其他关键字参数包括：
  - `edge`指定要计数的边沿。可以是`Counter.RISING`（默认）或 `Counter.FALLING`（支持ESP32）。
  - `direction`指定计数的方向。可以是`Counter.UP`（默认值）或`Counter.DOWN`（支持ESP32和MIMXRT）。可以用 machine.Pin 对象作为参数指定一个引脚控制计数方向：Low: Count up, High: Count down（支持MIMXRT）。
  - `filter_ns`指定了源信号需要稳定才能开始计数脉冲的最小时间（以纳秒为单位）。应使用小于或等于此值的硬件支持的最长过滤器。默认值为0（无筛选器）。（支持ESP32和MIMXRT）
  - `max`指定计数范围的上限。位置计数器将从最小起始值向上计数到最大值，然后滚动到初始值，并将循环计数器增加一个。倒计时时，循环计数器在从最小值到最大值的过渡处减小。通过将最大值和最小值都定义为0来重置范围。默认值是硬件的计数器范围。（由MIMXRT和ESP32 PCNT模块支持）
  - `min`指定最小的计数范围。默认值为0。
  - `index`指定索引脉冲连接的引脚。在索引脉冲的上升斜率处，脉冲计数器重置为最小值，循环计数器根据计数方向增加或减少1。值为None会禁用索引输入。（支持MIMXRT）
  - `reset`指定复位脉冲连接的引脚。在复位脉冲的上升斜率处，计数器设置为初始值，但循环计数器不变。如果值为None，则禁用重置输入。（支持MIMXRT）
  - `match`设置中断IRQ_match应触发的计数器值。不检查该值是否在计数器范围的边界内。此选项与ESP32 PCNT模块的阈值选项等效。值None重置匹配值并禁用IRQ_match中断。（支持MIMXRT）
  - `match_pin`指定匹配输出连接的引脚。只要计数器与匹配值匹配，此输出将具有高电平。该信号由编码器逻辑生成，不需要进一步的软件支持。脉冲宽度由输入信号频率定义，如果计数器停止在匹配值，脉冲宽度可以很短，如20ns。如果值为None，则禁用匹配输出。（支持MIMXRT）
<br><br>

- Counter.`deinit`()

  停止计数器，禁止中断并释放硬件资源。软复位将释放所有的计数器对象。
<br><br>

- Counter.`value`([value])

  获取或设置计数器值，作为带符号整数。实现必须以原子方式进行get和set（即不导致跳过计数）为目标。

  此计数器值可能超出小整数的范围，这意味着调用 `counter.value()` 可能会导致堆分配，但实现应旨在确保内部状态仅使用小整数，因此在调用 Counter.value() 之前，不会进行分配。

  例如，在ESP32上，硬件计数器的内部状态计数溢出（每32000次计数），这意味着在 $2^{30}*32000$ 次计数之前（在1MHz频率下略超过1年），它不会超过小整数范围。

  一般来说，建议使用 Counter.value(0) 重置计数器（即测量自上次调用以来的计数），这将避免此问题。
<br><br>

- Counter.`cycles`([value])

  获取或设置计数器的当前周期计数器为带符号的16位整数，该值表示计数范围的溢出或下溢事件。在没有参数的情况下，返回实际的循环计数器值。使用单值参数，循环计数器设置为该值，基本计数器不变。该方法返回上一个值。（支持MIMXRT）
<br><br>
...
- Counter.`irq`(handler=None, trigger=0, hard=False)
  ...
  事件可以为：
  - Counter.IRQ_RESET: 在复位输入发生转换时触发。
  - Counter.IRQ_INDEX: 在索引输入时触发转换。
  - Counter.IRQ_MATCH: 当位置计数器发生匹配时触发。对于快速信号，在回调中检索到的实际位置计数器值可能与触发值不同。
  - Counter.IRQ_ROLL_OVER: 当位置计数器从最高值翻转到最低值时触发。
  - Counter.IRQ_ROLL_UNDER: 当位置计数器从最低值翻转至最高值时触发。

  回调函数处理程序接收一个参数，即Counter对象。所有事件共享相同的回调，触发回调的事件可以用`irq.flags()` 方法标识。参数`hard`指定回调是作为硬中断调用还是作为常规调度函数调用。硬中断总是具有较短的延迟，但受到限制，因为它们不能分配内存。常规调度功能的使用不受限制，但根据设备的负载，执行可能会延迟。在低负载下，延迟的差异很小。

  默认参数值是 handler=None、trigger=0、hard=False。当使用 handler=None 时，回调将被禁用。

  只要位置和匹配值相同，就会触发位置匹配事件。因此，位置匹配回调以一次性方式运行，并且必须在位置发生变化时再次启用。它将通过使用 Counter.irq() 或 irq().trigger() 重新定义触发器来启用。对于ESP32，计数器中断由PCNT处理。（支持MIMXRT）
...
编码器通过根据两个输入脉冲的顺序向上或向下计数来实现对旋转编码器输出的正交信号的解码。

ESP32 基本用法：
```py
from machine import Pin, Encoder
encoder = Encoder(0, Pin(0, Pin.IN), Pin(1, Pin.IN)) # 使用引脚0和1创建编码器
value = encoder.value()                              # 获取当前计数值
```
...
- class machine.`Encoder`(id, ...)
  ...
  返回给定`id`的Encoder对象。id的值取决于特定硬件。值0、1等通常用于选择硬件模块#0、#1等。

  其他参数被传递给下面描述的`init()`方法，并将导致Encoder实例被重新初始化和重置。
...
- Encoder.`init`(phase_a, phase_b, *, ...)
  ...
  用给定参数初始化和重置编码器：
  - phase_a 将 machine.Pin 对象指定为第一个输入。
  - phase_b 将 machine.Pin 对象指定为第二个输入。

  如果给定硬件模块具有预定义引脚，可以省略这些引脚。

  不同硬件可能支持的其他关键字参数包括：
  - `filter_ns`指定了源信号需要稳定才能开始计数脉冲的最小时间段（以纳秒为单位）。应使用小于或等于此值的硬件支持的最长过滤器。默认值为0（无筛选器）。（支持ESP32和MIMXRT）
  - `phases`指定了要计数的信号边缘的数量，从而指定了解码的粒度。例如，4相位对应于“4x正交解码”，每个脉冲将产生四个计数。硬件可以支持1、2或4相位，默认为1相位。（支持ESP32和MIMXRT）
  - `max`指定计数范围的上限。位置计数器将从最小起始值向上计数到最大值，然后回滚到初始值，并将循环计数器增加一。倒计时时，循环计数器在从最小值到最大值的过渡处减小。通过将最大值和最小值都定义为0来重置范围。默认值是硬件的计数器范围。（由MIMXRT和ESP32 PCNT模块支持）
  - `min`指定计数范围的下限。默认值为0。（由MIMXRT和ESP32 PCNT模块支持）
  - `index`指定索引脉冲连接的引脚。在索引脉冲的上升斜率处，编码器计数器设置为最小值，循环计数器根据输入电平增加或减少1。值为None会禁用索引输入。（支持MIMXRT）
  - `reset`指定复位脉冲连接的引脚。在复位脉冲的上升斜率处，计数器设置为初始值，但循环计数器不变。如果值为None，则禁用重置输入。（支持MIMXRT）
  - `match`设置中断IRQ_match应触发的计数器值。不检查该值是否在计数器范围的边界内。此选项与ESP32 PCNT模块的阈值选项等效。值None重置匹配值并禁用IRQ_match中断。（支持MIMXRT）
  - `match_pin`指定匹配输出连接的引脚。只要计数器与匹配值匹配，此输出将具有高电平。该信号由编码器逻辑生成，不需要进一步的软件支持。脉冲宽度由输入信号频率定义，如果计数器停止在匹配值，脉冲宽度可以很短，如20ns。如果值为None，则禁用匹配输出。（支持MIMXRT）
<br><br>
...
- Encoder.`deinit`()
  ...
  停止编码器，禁止中断并释放硬件资源。软复位将释放所有的编码器对象。
<br><br>

- Encoder.`value`([value])
  
  获取或设置编码器值，作为带符号整数。实现必须以原子方式进行get和set（即不导致跳过计数）为目标。

  参考 `machine.Counter.value()` 获取此值溢出的详细信息。 
<br><br>
...
- Encoder.`irq`(handler=None, trigger=0, hard=False)
  ...
  事件可以为：
  - Encoder.IRQ_RESET: 在复位输入发生转换时触发。
  - Encoder.IRQ_INDEX: 在索引输入时触发转换。
  - Encoder.IRQ_MATCH: 当位置计数器发生匹配时触发。对于快速信号，在回调中检索到的实际位置计数器值可能与触发值不同。
  - Encoder.IRQ_ROLL_OVER: 当位置计数器从最高值翻转到最低值时触发。
  - Encoder.IRQ_ROLL_UNDER: 当位置计数器从最低值翻转至最高值时触发。

  回调函数处理程序接收一个参数，即Encoder对象。所有事件共享相同的回调，触发回调的事件可以用`irq.flags()` 方法标识。参数`hard`指定回调是作为硬中断调用还是作为常规调度函数调用。硬中断总是具有较短的延迟，但受到限制，因为它们不能分配内存。常规调度功能的使用不受限制，但根据设备的负载，执行可能会延迟。在低负载下，延迟的差异很小。

  默认参数值是 handler=None、trigger=0、hard=False。当使用 handler=None 时，回调将被禁用。

  只要位置和匹配值相同，就会触发位置匹配事件。因此，位置匹配回调以一次性方式运行，并且必须在位置发生变化时再次启用。它将通过使用 Encoder.irq() 或 irq().trigger() 重新定义触发器来启用。对于ESP32，编码器中断由PCNT处理。（支持MIMXRT）
