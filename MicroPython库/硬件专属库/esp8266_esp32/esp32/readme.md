...
提供对 ESP32 脉冲计数硬件功能的访问。ESP32 有 8个脉冲计数器单元，id为 0..7。

参考 `machine.Counter` 和 `machine.Encoder` 类，用于更简单、更便携地抽象常见的脉冲计数应用。这些类是作为围绕 `PCNT` 的精简Python实现。

- class esp32.`PCNT`(id, *, ...)

  返回给定 id 的 PCNT 实例。关键字参数传递给 `init()` 方法，如下所述。
<br><br>

- PCNT.`init`(*, ...)

  （重新）初始化脉冲计数器单元。支持的关键字参数包括：
  - `channel`：见下文说明
  - `pin`：用于监测脉冲的输入引脚
  - `rising`：上升沿的行动 —— `PCNT.INCREMENT`、`PCNT.DECREMENT` 或 `PCNT.IGNORE`（默认值）
  - `falling`：下降沿的行动（上升沿保存的参数）。
  - `mode_pin`：ESP32脉冲计数器支持监测第二个引脚，并根据其电平更改计数器的行为 —— 此关键字可设置为任何输入引脚
  - `mode_low`：当 `mode_pin` 为低电平时，设置为 `PCNT.HOLD` 或 `PCNT.REVERSE`，可暂停计数或反转计数器的方向（即 `PCNT.INCREMENT` 的行为与 `PCNT.DECREMENT` 相同，反之亦然）
  - `mode_high`：与 `mode_low` 相同，但用于 `mode_pin`为高时
  - `filter`：设置值范围 1..1023，以80MHz时钟的节拍为单位，以启用脉宽滤波器
  - `min`：递减时设置为计数器值的最小值（-32768…-1），或设置为0禁用
  - `max`：递增时设置为计数器值的最大值（1..32767），或设置为0禁用
  - `threshold0`：设置 `PCNT.IRQ_THRESHOLD0` 事件的计数器值（见IRQ方法）
  - `threshold1`：设置 `PCNT.IRQ_THRESHOLD1` 事件的计数器值（见IRQ方法）
  - `value`：可以设置为0以重置计数器

  硬件初始化是分阶段完成的，因此一些关键字参数可以分组或单独使用，以部分重新配置一个单元：
  - `pin` 关键字（可选与 `mode_pin` 组合）可用于仅更改绑定的pin
  - `rising`、`falling`、`mode_low`和`mode_high`可以（单独或一起）用于更改计数逻辑
    - 省略的关键字使用其默认值（`PCNT.IGNORE`或`PCNT.NORMAL`）
  - `filter` 只能用于更改脉宽滤波器（0代表禁用）
  - `min`、`max`、`threshold0`和`threshold1`中的每一个都可以单独更改这些限制/事件值；但是，任何设置都会将计数器重置为零（即 `value=0`）

  每个脉冲计数器单元支持两个通道，0和1，每个通道都能够用不同的计数逻辑监测不同的引脚，但更新相同的计数器值。使用 `channel=1` 和 `pin`、`rising`、`falling`、`mode_pin`、`mode_low` 和 `mode_high` 关键字来配置第二个通道。
  
  第二信道可用于配置具有单个计数器单元的4X正交解码：
...
- PCNT.`value`([value])
  ...
  如果可选 `value` 参数设置为0，则重置计数器（但返回之前的值）。读取和重置不是原子性的，因此可能会错过脉冲。除 0 之外的任何值都会引发错误。
<br><br>

- PCNT.`irq`(handler=None, trigger=PCNT.IRQ_ZERO)

  ESP32脉冲计数器在以下计数器事件中支持中断：
  - `PCNT.IRQ_ZERO`：计数器已重置为零
  - `PCNT.IRQ_MIN`：计数器已达到最小值
  - `PCNT.IRQ_MAX`：计数器已达到最大值
  - `PCNT.IRQ_THRESHOLD0`:计数器已达到阈值0
  - `PCNT.IRQ_THRESHOLD1`:计数器已达到阈值1

  `trigger` 是所需事件或同时发生的事件的位掩码。`handler` 函数接受一个参数，即引发事件的`PCNT`实例。

  此方法返回一个回调对象，回调对象可用于访问`PCNT`单元上未完成事件的位掩码。
  ...
  **注意**：访问`irq.flags()`将清除标志，因此每次处理程序时只调用一次。

  处理程序是通过MicroPython调度程序调用的，因此将在中断后的某个时间点运行。如果在调用处理程序之前发生另一个中断，则事件将合并为一个调用，位掩码将指示已发生的所有事件。

  为了避免被调用的处理程序和检索当前计数器值之间的竞争条件，`value()` 方法将在返回当前计数器值（并可能重置该值）之前强制执行任何未决事件。

  每个单元只能有一个处理程序。将 `handler` 设置为 `None` 以禁用事件中断。

**®注意事项**: ESP32脉冲计数器在达到最小值或最大值时重置为零。因此，当这些事件中的任何一个发生时，`IRQ_ZERO`事件也将触发。

参考 `machine.Counter` 和 `machine.Encoder` 类，用于更简单地抽象常见的脉冲计数应用。
